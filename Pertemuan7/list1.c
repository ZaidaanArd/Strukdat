...
void PrintList(List1 L)
{
	...
	if (P != NIL)
	{
		...
		do
		{
			printf("\t%c", info(P));
			P = next(P);
		} while (P != NIL); // perulangan selama masih ada elemen berikutnya
	}
}
...
int NbElm(List1 L)
{
	...
	count = 0;
	P = First(L);
	while (P != NIL) // selama P tidak kosong, lanjut ke elemen berikutnya
	{
		count++;
		P = next(P);
	}
...
void InsertVLast(List1 *L, infotype V)
{
	...
	{ // alokasi berhasil
		...
		{
			Q = First(*L);
			while (next(Q) != NIL) // bergerak sampai menemukan elemen terakhir
			{
				Q = next(Q);
			}
			next(Q) = P;
...
void DeleteVLast(List1 *L, infotype *V)
{
	...
	if (!IsEmptyList(*L))
	{
		...
		{
			Q = First(*L);
			while (next(next(Q)) != NIL) // berhenti di elemen sebelum elemen terakhir
			{
				Q = next(Q);
			}
			P = next(Q);
...
void SearchX(List1 L, infotype X, address *A)
{
	...
	P = First(L);
	*A = NIL;
	while (P != NIL) // telusuri list sampai ketemu X atau habis
	{
		if (info(P) == X)
		{
			*A = P;
...
void UpdateX(List1 *L, infotype X, infotype Y)
{
	...
	// algoritma
	P = First(*L);
	while (P != NIL) // cek semua elemen dalam list
	{
		if (info(P) == X)
		{
			info(P) = Y;
...
void Invers(List1 *L)
{
	...
	Prev = NIL;
	P = First(*L);
	while (P != NIL) // membalik arah next hingga akhir list
	{
		Next = next(P);
		next(P) = Prev;
		Prev = P;
...
int CountVocal(List1 L)
{
	...
	count = 0;
	P = First(L);
	while (P != NIL) // telusuri semua elemen, cek apakah vokal
	{
...
int CountX(List1 L, infotype X)
{
	...
	count = 0;
	P = First(L);
	while (P != NIL) // hitung berapa kali X muncul
	{
		if (info(P) == X)
		{
			count++;
...
void SearchAllX(List1 L, infotype X)
{
	...
	while (P != NIL) // cek tiap elemen dan tulis posisinya jika sama dengan X
	{
		if (info(P) == X)
		{
			printf("%d ", position);
...
void UpdateAllX(List1 *L, infotype X, infotype Y)
{
	...
	// algoritma
	P = First(*L);
	while (P != NIL) // telusuri semua elemen dan ganti X dengan Y
	{
		if (info(P) == X)
		{
			info(P) = Y;
...
void InsertVAfter(List1 *L, infotype V, infotype VA)
{
	...
	// algoritma
	Q = First(*L);
	while (Q != NIL) // cari elemen dengan info V
	{
		if (info(Q) == V)
		{
			P = Alokasi(VA);
...
infotype Modus(List1 L)
{
	...
	maxCount = 0;
	P = First(L);
	while (P != NIL) // untuk setiap elemen, hitung frekuensinya
	{
...
int NbModus(List1 L)
{
	...
	count = 0;
	P = First(L);
	while (P != NIL) // hitung berapa banyak modus muncul di list
	{
		if (info(P) == modus)
		{
			count++;
...
void ConcatList(List1 L1, List1 L2, List1 *L)
{
	...
	CreateList(L);
	P = First(L1);
	while (P != NIL) // salin semua elemen dari L1 ke L
	{
		InsertVLast(L, info(P));
		P = next(P);
	}
	P = First(L2);
	while (P != NIL) // lanjut salin semua elemen dari L2 ke L
	{
		InsertVLast(L, info(P));
		P = next(P);
	}
...
void SplitList(List1 L, List1 *L1, List1 *L2)
{
	...
	P = First(L);
	i = 1;
	while (P != NIL) // bagi elemen: indeks ganjil ke L1, genap ke L2
	{
		if (i % 2 == 1)
		{
...
void CopyList(List1 L1, List1 *L2)
{
	...
	CreateList(L2);
	P = First(L1);
	while (P != NIL) // salin semua elemen dari L1 ke L2
	{
		InsertVLast(L2, info(P));
		P = next(P);
	}
